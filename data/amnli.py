...
def load_data(args, lang):
    ...
    val = pd.read_parquet(os.path.join(args["data_dir"], "amnli", "amnli.dev.parquet"))
    val = val[val["language"] == lang].reset_index(drop=True)[["premise", "hypothesis", "label"]].to_dict(orient="list")
    train = val.copy()

    test = pd.read_parquet(os.path.join(args["data_dir"], "amnli", "amnli.test.parquet"))
    if lang in test["language"].unique():
        test = test[test["language"] == lang].reset_index(drop=True)[["premise", "hypothesis", "label"]].to_dict(orient="list")
    else: 
        test = None
